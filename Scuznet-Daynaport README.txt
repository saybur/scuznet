...
-----------------------
Not documented by Anodyne but gets called when AppleTalk is turned on.  If the device doesn't respond by reading from the Mac whatever data is sent (length is specified in bytes 4 and 5 of the command block) AppleTalk initialization fails on the Mac.  Updated firmware only allows multicast packets via ENC filter if AppleTalk is enabled by the driver and if AppleTalk is enabled, the updated driver filters out all non AppleTalk multicast packets to improve performance.

FILTERING
~~~~~~~~~			
Scuznet-Daynaport network filter only allows Multicast packets through (at the ENC level) if the driver turns on AppleTalk.  If AppleTalk is turned on, the driver only allows AppleTalk multicast packets through and discards all others.